...
namespace vm
{
	...
	static inline atomic_t<reservation_waiter_t, 128>* reservation_notifier(u32 raddr, u64 rtime)
	{
		...
		// Keying on rtime as well as the address avoids false collisions between reservation
		// generations, so notifications are precise (no spurious wakeups) and never stranded.
